...
class AuditRecord(BaseModel):
    ...
    def calculate_checksum(self, secret_key: bytes) -> str:
        ...
        data = f"{self.id}{self.timestamp.isoformat()}{self.event_type}{self.client_id_hash}{self.success}"
        if self.ip_address:
        ...
        try:
            with self._lock:
                ...
                try:
                    ...
                    conn.execute(
                        """
                        INSERT INTO database_audit (timestamp, operation, table_name, record_id, checksum)
                ...
                try:
                    ...
                    for row in rows:
                        ...
                        if record.verify_checksum(self.encryption_key):
                            ...
                            logger.warning(
                                f"Audit record integrity check failed: {record.id}"
                            )
        ...
        try:
            with self._lock:
                ...
                try:
                    ...
                    conn.execute(
                        """
                        INSERT INTO database_audit (timestamp, operation, table_name, record_id)
...
class AuditLogger:
    ...
    def log_authentication_event(
        ...
        try:
            ...
            record_id = f"{self._session_prefix}_{self._session_counter:06d}_{secrets.token_urlsafe(8)}"
    ...
    def _generate_recommendations(
        ...
        if high_risk_events:
            ip_addresses = set(e.ip_address for e in high_risk_events if e.ip_address)
            if len(ip_addresses) > 3:
                recommendations.append(
                    "Authentication attempts from multiple IP addresses - consider IP restrictions"
                )
